...
Vi sao co cai nay khi da co BrowserAdapter: do that 07/09/2026 tren proxy dan cu -
...
from seeding.adapters.browser import COMMENT_RECIPES, RECIPES, BrowserAdapter
from seeding.config import get_settings
from seeding.core.signer import Signer, SignerNotReady
from seeding.models import Account, Platform, Profile, Proxy, Variant
...
def caption_for(variant: Variant) -> str:
    """Cung cach ghep title/body voi BrowserAdapter, de doi duong khong doi bai."""
...
class TikTokHttpAdapter:
    """Dang bai TikTok bang HTTP; binh luan uy quyen cho BrowserAdapter."""
    ...
    async def publish(
        ...
        *,
        ...
        if kind == "comment":
            return await self.browser.publish(account, variant, target, profile=profile)
...
# Ghi de adapter trinh duyet cho TikTok - phai import SAU adapters.browser (tasks.py
# lam dung thu tu do). Tat bang TIKTOK_POST_VIA_HTTP=false de quay ve trinh duyet.
if get_settings().tiktok_post_via_http:
    register(
        TikTokHttpAdapter(
            browser=BrowserAdapter(
                Platform.TIKTOK, RECIPES[Platform.TIKTOK], COMMENT_RECIPES.get(Platform.TIKTOK)
            )
        )
    )
