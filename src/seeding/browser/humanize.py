"""Nhip thao tac giong nguoi that.
...
async def type_like_person(
    ...
    *,
    ...
    """Go tung ky tu voi do tre thay doi, nghi lau hon o dau cau.

    `cps` la so ky tu moi giay trung binh. Nguoi go nhanh khoang 6-8.
    """
...
async def scroll_feed(
    ...
    *,
    ...
    """Cuon feed vai nhip, thinh thoang cuon nguoc len.

    Tra ve so nhip da cuon. Doi khi cuon nguoc la co y: nguoi that hay luot qua roi
    quay lai xem lai.
    """
    rng = rng or random.Random()
    done = 0
...
async def warm_up(
    ...
    *,
    ...
    """Xem qua feed truoc khi lam gi do.

    Mot tai khoan vao trang la dang bai ngay roi thoat la mau hinh bot ro nhat.
    Ham nay chay truoc MOI lan dang bai, khong phai tuy chon.
    """
    rng = rng or random.Random()
