...
from seeding.adapters import tiktok_http as mod
from seeding.core.signer import SignerNotReady
from seeding.models import Account, AccountStatus, Platform, Profile, Proxy, Variant
...
class RecordingBrowser:
    ...
    async def publish(self, account, variant, target, *, profile=None):
        self.calls.append(target)
        return mod.PublishResult(ok=True, remote_url="browser://comment")
...
def _adapter(
    ...
    return mod.TikTokHttpAdapter(
        browser=browser or RecordingBrowser(),
...
async def test_comments_are_delegated_to_the_browser_adapter(monkeypatch, tmp_path):
    tiktok = FakeTikTok()
    browser = RecordingBrowser()
    result = await _adapter(tiktok, browser=browser).publish(
        _account(),
        _variant(tmp_path),
        {"kind": "comment", "url": "https://www.tiktok.com/@x/video/1"},
        profile=_profile(monkeypatch),
    )
    assert result.ok and result.remote_url == "browser://comment"
    assert browser.calls == [{"kind": "comment", "url": "https://www.tiktok.com/@x/video/1"}]
    assert tiktok.requests == []
