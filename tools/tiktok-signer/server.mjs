...
// User agent the signing page runs under. X-Gnarly hashes navigator.userAgent, so
// every request sent with a signature from here MUST carry this exact UA.
//
// [va cuc bo] Doc tu SIGNER_USER_AGENT de khop voi UA cua doi tai khoan. Cookie cua
// cac profile duoc tao duoi Firefox/Windows (Camoufox); ky bang UA Safari/macOS roi
// gui request voi UA do la doi danh tinh giua chung phien - khong can thiet.
const DEFAULT_UA =
...
async function _generateSignedUrlInternal(
  ...
  // [va cuc bo] KHONG xoa msToken o day. Neu nguoi goi da dat msToken cua TAI KHOAN
  // trong URL thi chu ky phai duoc tinh tren dung query do; _signDirectly chi bu
  // msToken cua trinh duyet ky khi URL khong co san.
...
async function _signDirectly(fetchUrl, userAgent = null) {
  const out = await page.evaluate((url) => {
    ...
    // [va cuc bo] Neu URL da mang msToken cua TAI KHOAN thi giu nguyen. Ghi de bang
    // msToken cua trinh duyet ky la lam lech chu ky khoi query that duoc gui di.
    if (!u.searchParams.has("msToken")) u.searchParams.set("msToken", msToken);
